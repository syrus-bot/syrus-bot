...
module.exports = class ClientCommand extends SyrusCommand {
	...
	async run(message, args) {
		...
		if (!canEmbed) {
			try {
				...
				await message.replyTranslated("core:help.dm");
			} catch {
				await message.replyTranslated("core:help.nodm");
			}
		}
	...
	async buildHelp(message, categories, individual) {
		...
		const fields = await Promise.all(commands.map(async (category) => {
			const commandList = await Promise.all(
				category.map(async (command) => {
					const description = await message.resolveKey(
						command.description
					);
		...
		const allHelpDesc = `Hover over commands for more info (on PC), OR
use \`${prefix}help [command|category]\``;
	...
	async buildCommandEmbed(message, command) {
		...
		const detailed = await message.resolveKey(toDetail);
